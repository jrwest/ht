...
* Cascade Improvements
  * Allow cascades to be "reopened" like a ruby class and modified
  * Delete layer by name
  * Update layer dependency by name
  * Cascade mixins? Mix cascades into each other? or have seperate structure?
  * Give cascade names a purpose
    * Global cascade hash/table
    * Build by name instead of cascade instance
    * Aide in opening existing templates and modifying them
      * calling new with the name of an existing template can modify or overwrite

* Build Process Improvements
  * skip layer (skip)
  * halt/stop (stop execution before or after this layer or on call) 
  * priority/freeze (mark a value is no longer able to be overwritten during build)
